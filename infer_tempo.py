...
class InferTempo:
    ...
    def build_beat_candidates(
        self, motion_angles: list, threshold: float = 5.0
    ) -> list:
        """
    ...
    def infer_faircloth_tempo(
        ...
        for vote in candidates:
            ...
            for agent in agents:
                ...
                while (
                    vote[0] - agent["history"][len(agent["history"]) - 1][0]
                ) > timeout:
                    new_beat_onset = (
                        agent["history"][len(agent["history"]) - 1][0]
                        + agent["interval"]
                    )
                ...
                if (agent["prediction"] - pre_tolerance <= vote[0]) and (
                    vote[0] <= agent["prediction"] + post_tolerance
                ):
        ...
        for agent in agents:
            agent["score"] = agent["score"] * (
                (1.0 - ((agent.get("penalties") * agent.get("interval")) / num_frames))
                * agent.get("matches")
                / num_candidates
            )
